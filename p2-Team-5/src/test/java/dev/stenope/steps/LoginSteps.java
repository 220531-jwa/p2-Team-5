...
public class LoginSteps {
	 ...
	 * Scenario 2
	 */
	@When("the User inputs an incorrect {string} and {string} and clicks the login button")
	public void the_user_inputs_an_incorrect_test_name_or_test_pass(String wrongU, String wrongP) {
		loginPage.uName.sendKeys(wrongU);
	    loginPage.pKey.sendKeys(wrongP);
	    loginPage.loginButton.click();
	}

	@Then("the User is given an error message")
	public void the_user_is_given_an_error_message() {
		Alert invalidLogin = new WebDriverWait(realHumanBeing, Duration.ofSeconds(10))
		.until(ExpectedConditions.alertIsPresent());
		
		assertEquals(invalidLogin.getText(), "Login failed");
	}
}
